...
namespace PLCConnect
{
    ...
    public class ModbusRTUClient
    {
        ...
        private string mConnectStatusDisplay = "";

        public bool IsConnected { get; private set; }
        public string pConnectStatusDisplay {
            get { return mConnectStatusDisplay; }
            set {
                mConnectStatusDisplay = value;
                OnConnectedStatusChanged?.Invoke(mConnectStatusDisplay);
            } }

        public event Action<ushort[]> OnDataChanged;
        public event Action<string> OnConnectedStatusChanged;
        ...
        public ModbusRTUClient(byte slaveId, string portName, int baudRate, int readTimeout, int writeTimeout)
        {
            ...
            ManagerInfor.Instance.PropertyGridObj.portName = portName;
            ManagerInfor.Instance.PropertyGridObj.baudrate = baudRate;
            ManagerInfor.Instance.PropertyGridObj.readTimeout = readTimeout;
            ManagerInfor.Instance.PropertyGridObj.writeTimeout = writeTimeout;

            _serialPort = new SerialPort(portName)
            {
                BaudRate = ManagerInfor.Instance.PropertyGridObj.baudrate,
                DataBits = ManagerInfor.Instance.PropertyGridObj.dataBits,
                Parity = ManagerInfor.Instance.PropertyGridObj.parity,
                StopBits = ManagerInfor.Instance.PropertyGridObj.stopBits,
                WriteTimeout = ManagerInfor.Instance.PropertyGridObj.readTimeout,
                ReadTimeout = ManagerInfor.Instance.PropertyGridObj.writeTimeout,
            };
            //_serialPort = new SerialPort(portName)
            //{
            //    BaudRate = baudRate,
        ...
        public bool Connect()
        {
            try {
                ...
                IsConnected = true;
                pConnectStatusDisplay = "Connected";
                Console.WriteLine($"PLC {pConnectStatusDisplay} {_serialPort.PortName}");
            ...
            {
                pConnectStatusDisplay = "Disconnected";
        ...
        private void StartReconnectMonitor()
        {
            ...
            _reconnectThread = new Thread(() =>
            {
                while (true) {
                    if (!IsConnected) {
                        try {
                            pConnectStatusDisplay = "Reconnecting";
                            ...
                            pConnectStatusDisplay = "Reconnect success";
                            Console.WriteLine($"{pConnectStatusDisplay}");
                        }
                        catch
                        {
                            pConnectStatusDisplay = "Reconnect Fail";
                            Console.WriteLine($"{pConnectStatusDisplay}");
                        }
                    }
